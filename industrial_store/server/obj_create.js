...
const generateData = async () => {
  ...
  const workSchedules = Array.from({ length: 100000 }).map(() => ({
    day: new Date(new Date().getTime() - Math.floor(Math.random() * 10000000000)).toISOString(), 
  ...
  const supplies = Array.from({ length: 50000 }).map(() => ({
    ...
    deliveredAt: new Date(new Date().getTime() - Math.floor(Math.random() * 10000000000)).toISOString(), 
  }));

  const sales = Array.from({ length: 75000 }).map(() => ({
    totalAmount: Math.floor(Math.random() * 9901) + 100,
    saleDate: new Date(new Date().getTime() - Math.floor(Math.random() * 10000000000)).toISOString(), 
